...
def weather_icon_generator():
    weather_icon = ImageTk.PhotoImage(file="Clouds.png")
...
def submit_output():
    ...
    if city_name:
        ...
        if response.json():
            ...
            data["temp"] = int(forecast_data.json()["main"]["temp"] - 273.15)
            data["humidity"] = forecast_data.json()["main"]["humidity"]
            converted_json_data = f"City name: {data['name']}, Weather: {data['weather']}, Temperature: {data['temp']}°C, Humidity: {data['humidity']}%"

            TextAreaSingleOutput.delete(1.0, END)
            weather_icon = ImageTk.PhotoImage(file=f"icons/{data['weather']}.png")
            TextAreaSingleOutput.image_create(END, image=weather_icon)
            TextAreaSingleOutput.image = weather_icon
...
def generate_output():
    ...
    # Keep reference of each image, bug with TkInter ->
    ...
    for idx in range(5):
        ...
        weather_icon = ImageTk.PhotoImage(file=f"icons/{weather}.png")
        reference.append(weather_icon)
        TextAreaCities.image_create(END, image=weather_icon)
    ...
    # Instantiate each icon
    TextAreaCities.image = [s for s in reference]
...
ttk.Label(root, text="Weather App", font=("MS Serif bold", 42), background="#AEE2FF", foreground="black").place(relx=.5,
                                                                                                                rely=.1,
                                                                                                                anchor=CENTER)

entry = ttk.Entry(root, width=39)
entry.place(relx=.5, rely=.2, anchor=CENTER)

ttk.Button(root, text="Submit", command=submit_output).place(relx=.37, rely=.3, anchor=CENTER)
ttk.Button(root, text="Generate", command=generate_output).place(relx=.65, rely=.3, anchor=CENTER)

TextAreaCities = Text(root, height=10, width=65, bg="#D0F0C0", foreground="black", borderwidth=0)
TextAreaCities.configure(font=font, state=DISABLED)

TextAreaColdestCity = Text(root, height=1.5, width=65, bg="#D0F0C0", foreground="black", borderwidth=0)
TextAreaColdestCity.configure(font=font, state=DISABLED)

TextAreaAvgTemp = Text(root, height=1.5, width=65, bg="#D0F0C0", foreground="black", borderwidth=0)
TextAreaAvgTemp.configure(font=font, state=DISABLED)

TextAreaSingleOutput = Text(root, height=4, width=65, bg="#D0F0C0", foreground="black", borderwidth=0)
